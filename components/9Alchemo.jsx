...
function Alchemo() {
  ...
  const [text, setText] = useState('Alchemie');
  const [previousText, setPreviousText] = useState('Grouping'); // Speichert den vorherigen Zustand
  ...
  const MAX_FONT_SIZE = 16; // Maximale Schriftgröße
...
Streetlights turn on one by one, casting a yellowish glow on wet asphalt. Reflections of signs and buildings create fragments of color on the surface of the road. The air cools down, and a faint smell of rain lingers in the streets. People continue walking, their paces varying—some rushing, others taking their time. The fox remains a silent observer, moving unseen through the cityscape. Numbers flicker on screens: 32 departures, 19 arrivals. A tram slows at a station, doors opening for the next wave of passengers.`;
  ...
  useEffect(() => {
    const adjustTextSize = () => {
      ...
      tempSpan.style.fontFamily = 'Graber, sans-serif';
  ...
  const weightOptions = [100, 300, 400, 500, 600, 700, 900 ];   //Hier Zahlenwerte vorgeben, im Weight dropdown benennen
  ...
  const stylisticSetOptions = [
    { value: "ss01", label: "Stylistic Set 1" },
    { value: "ss02", label: "Stylistic Set 2" },
    { value: "ss03", label: "Stylistic Set 3" },
    { value: "ss04", label: "Stylistic Set 4" },
    { value: "none", label: "Standard" }, // Standard ohne Stylistic Set
  ];
    ...
    >
      <section className="input-section">
        ...
        onChange={(e) => setFontWeight(getClosestWeight(parseInt(e.target.value, 10)))}
          ...
          <option key={weight} value={weight}>{ weight === 100 ? 'Ultralight' : weight === 300 ? 'Thin' : weight === 400 ? 'Regular' : weight === 500 ? 'Medium' : weight === 600 ? 'Semiold' : weight === 700 ? 'Bold' : weight === 900 ? 'Black' : weight } </option>
          ))}
        ...
        <div className="input-wrapper">
          <textarea
            ...
            style={{
              ...
              lineHeight: isLoremActive ? "1.5" : "0.9",  // Dynamische Line Height
              padding: isLoremActive ? "0px 18rem 0px 18rem" : " 4.5rem 5rem 1.5rem 5rem",   // Unterschiedliches Padding je nach Modus ######################
...
<button className="styled-button align-center-button" onClick={alignTextCenter}>
  ...
  <div className="weight-buttons">
    {['100', '400', '700', '900'].map((weight, index) => (
      ...
      <div id="top-section-alchemo">
        <section id="displaySection">
          <div id="displayLetter-alchemo" className={isGlowing ? 'glow' : ''}
          style={{ fontWeight }}
          >
            {displayLetter}
        ...
        <section id="alphabetPanel-alchemo">
